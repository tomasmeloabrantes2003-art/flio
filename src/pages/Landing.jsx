  ...
  return (
    <div className="min-h-screen bg-base-950 flex flex-col">
      <nav className="flex items-center justify-between px-5 py-4 max-w-5xl w-full mx-auto">
        <span className="text-[15px] font-semibold text-white tracking-tight">flio</span>
        <Link
          to="/app"
          className="text-[13px] text-base-400 hover:text-white transition"
        >
          Open app
        </Link>
      </nav>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-5 pb-20">
        <div className="max-w-lg w-full">
          <p className="text-[13px] text-blue-400 font-medium mb-3 tracking-wide uppercase">
            Travel alerts, simplified
          </p>
          <h1 className="text-[32px] sm:text-[40px] font-semibold text-white leading-[1.15] tracking-tight">
            Know when to book.
          </h1>
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit} className="mt-6 flex gap-2 max-w-sm">
              <input
                ...
                className="flex-1 px-3 py-2 text-[13px] rounded-md bg-base-800 border border-base-600 text-white placeholder-base-500 focus:outline-none focus:border-blue-500 transition"
              />
              <button
                type="submit"
            ...
            <p className="mt-6 text-[13px] text-green-400">You're in. We'll email you when it's ready.</p>
          )}
